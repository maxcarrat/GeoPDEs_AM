% MATRIX_BASIS_CHANGE_COARSENING: compute the subdivision matrix between two consecutive levels.
%        This method is intended to remain private.
%
% function C = matrix_basis_change_coarsening (hspace, lev)
%
% Compute the new matrices to approximate functions of level "lev"
% with a linear combinations of splines (active and inactive) of level "lev-1"
%
% INPUT:  
%
...
% OUTPUT:
%
%   C:    matrix to change basis from level lev to level lev-1
%
...
function C = matrix_basis_change_coarsening (hspace, lev, ind_coarse)
...
%   Wlev_coarse = spdiags (1./hspace.space_of_level(lev).weights(:), 0, hspace.space_of_level(lev).ndof, hspace.space_of_level(lev).ndof);
%   C = Wlev_coarse * C * Wlev;
% end
% 
% if (hspace.truncated)
