function [rcrv, t, R] = basiskntrem( n, p, U, Pw, u, r, s, num, d )
%BASISKNTREM : Remove one knot from the knot-vector of a NURBS curve.
% 
% Calling Sequence:
% 
%   [rcrv, remflag] = crvkntremove (n, p, U, Pw, u, r, s, num, d);
% 
% INPUT:
% 
...
%    along with this program.  If not, see <http://www.gnu.org/licenses/>.
  ...
  [R, U, Pw, t] = RemoveCurveKnot (n, p, U, Pw, u, r, s, num, d);
...
function [R, U, Pw, t] = RemoveCurveKnot (n, p, U, Pw, u, r, s, num, d)
...
%   TOL = d*w / (1 + Pmax);
  ...
  ncps = size(Pw,2);
  R = zeros(ncps-num, ncps);
  R(1:first-1, 1:first-1) = eye(first-1);
  ...
  for t = 0:num-1
    ...
    while (j - i > t)
      ...
      R(i ,i) = 1/alfi;
      R(i ,off+ii-1+1-1) = -(1.0-alfi)/alfi;
      
      R(j ,j) = 1/(1.0-alfj);
      R(j ,off + jj+1+1-1) = -(alfj)/(1.0-alfj);
