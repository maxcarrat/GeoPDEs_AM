...
function [eu, F] = sp_eval_msh (u, space, msh, options, lambda_lame, mu_lame)
  ...
  F = [];
  if isfield(msh, 'geo_map'); F = msh.geo_map; end
    

  %% HACK
  % if called for non-linear HBsplines space.connectivity~=0 execeds the
  % dofs vector index!!!
  
  uc = zeros (size (space.connectivity));
  uc(space.connectivity<=numel(u)) = ...
        u(space.connectivity(space.connectivity<=numel(u)));
